...
本サイトでは、公開して終わりではなく、改善を前提とした運用ができるように
以下の計測基盤を構築しています。

- Google Analytics 4
  ページ閲覧・流入元・ユーザー行動を計測し、記事や導線改善の判断に利用。

- Google Tag Manager
  計測タグをコードから分離し、将来的なイベント追加や変更に対応できる構成。

- Microsoft Clarity
  ヒートマップ・セッション録画により、数値だけでは分からないユーザー行動を把握。

- Google Search Console
  検索流入・クエリ・表示回数を把握し、記事改善やSEOの判断材料として使用。

- IP 除外設定
  自身のアクセスを除外し、実ユーザーの行動のみを計測。
